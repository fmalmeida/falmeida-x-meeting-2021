...
This idea is being reinforced with the ongoing genomic surveillance of the virus during the Sars-CoV-2 pandemics 
]
...
.bg-washed-blue.b--dark-blue.ba.bw2.br3.shadow-5.ph4.mt0[
These pose a great hindrance to the implementation of genomics routines
]
...
In order to address this need, we developed three sequential pipelines that together, as we can see in the image, provide a comprehensive workflow for raw data preprocessing, genome assembly and bacterial genome annotation.
...
* The pipelines were implemented in Nextflow and all the tools are wrapped inside docker containers.

* The distribution is made via Github and Dockerhub, which makes installation be quick and easy, only being required to download the pipeline and the docker images.
...
* The simplest use is via a configuration file which will point to where your input data is, and a few configurations where you set some parameters such as: the input data type, quality thresholds, steps to skip, etc.

* All parameters can be set via the command line, however it is easier and cleaner to use the configuration file which is exemplified in the image on the right.
...
* All that while providing nice alternatives for results visualization and interpretation, such as: HTML reports, a genome browser, and a web app for "interrogation" of all the results

* Although comprehensive, the pipeline is modular, meaning that users can skip annotation steps if desired.

* Which also makes the pipeline highly adaptable to the community needs, meaning that new steps and modules can be rapidly added without because it does not require to "change" the whole pipeline.
