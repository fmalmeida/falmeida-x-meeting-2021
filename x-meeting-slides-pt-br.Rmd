...
* Tudo isso enquanto providenciando ótimas alternativas para a visualização, interpretação e interrogação dos resultados. Como por exemplo:
...
* O último, foi a análise de uma cepa de solo de cerrado que, através dos resultados dos pipelines e principalmente das anotações funcionais fizemos uma vasta caracterização genômica e fisiológica para compará-la com espécies próximas e propor uma nova espécie e a existência de um cromídeo.
...
* Como perspectivas principais para a segunda parte do doutorado nós temos:
  ...
  + Realizar a análise de 32 cepas isoladas no HuB que possuem contextos interessantes de fenótipos de resistência e virulência não usuais. Estas já foram mandadas para sequenciamento.
  + Utilizar o pipeline de montagem como base para preparação de dados de Renseq de algumas plantas em colaboração com a EMBRAPA.
